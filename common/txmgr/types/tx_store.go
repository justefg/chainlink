...
type TransactionStore[
	...
	// UpdateTxFatalError updates transaction states to fatal error, deletes attempts, and clears broadcast info and sequence
	UpdateTxFatalError(ctx context.Context, etx *Tx[CHAIN_ID, ADDR, TX_HASH, BLOCK_HASH, SEQ, FEE]) error
	// UpdateTxTerminallyStuck updates transaction states to fatal error with error message
	UpdateTxTerminallyStuck(ctx context.Context, etxID []int64, errMsg string) error
