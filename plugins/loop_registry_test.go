...
func TestLoopRegistry_Register(t *testing.T) {
	...
	require.Equal(t, &url.URL{Scheme: "fake", Host: "database.url"}, cfg.DatabaseURL)
	...
	require.Equal(t, 42, cfg.DatabaseMaxOpenConns)
	require.Equal(t, 99, cfg.DatabaseMaxIdleConns)
}
