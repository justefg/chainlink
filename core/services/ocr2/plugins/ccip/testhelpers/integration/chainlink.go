...
func setupNodeCCIP(
	...
	loopRegistry := plugins.NewLoopRegistry(lggr.Named("LoopRegistry"), config.Tracing(), config.Database())
	...
	app, err := chainlink.NewApplication(chainlink.ApplicationOpts{
		...
		MailMon:                    mailMon,
		LoopRegistry:               plugins.NewLoopRegistry(lggr, config.Tracing(), config.Database()),
	})
	require.NoError(t, err)
